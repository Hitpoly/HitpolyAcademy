...
export const useEnrollmentLogic = (urlSlugFromParams) => {
      ...
      try {
        ...
        if (courseResult.found) {
          ...
          if (isAuthenticated && user?.id && courseResult.details.title) {
            ...
            console.log(`[ACADEMY DEBUG] ¿Usuario ${user.id} está inscrito en "${courseResult.details.title}"?:`, enrolled);
  ...
  const handleRegisterAndEnroll = async (
    ...
    try {
      ...
      setIsEnrolled(true); // Actualizar estado inmediatamente
      setOfferDialogOpen(true); // Abrir popup de oferta tras registro exitoso
      resetForm();
      } catch (error) {
  ...
  const handleUpdateInterestAndEnroll = async (
    ...
    try {
      ...
      setIsEnrolled(true); // Actualizar estado inmediatamente
      setOfferDialogOpen(true); // Abrimos el popup de oferta
      resetForm();
      } catch (error) {
      showToast("error", `Error en el proceso: ${error.message}`);
      } finally {
