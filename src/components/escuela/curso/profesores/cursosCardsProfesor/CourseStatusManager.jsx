...
import OfferManagerDialog from "./OfferManagerDialog";
...
const CourseStatusManager = ({
  ...
  const [selectedCourseForOffer, setSelectedCourseForOffer] = useState(null);
  ...
  // ==========================================
  // LOGS SOLICITADOS POR EL USUARIO PARA EL PROFESOR
  // ==========================================
  useEffect(() => {
    if (!loading && allCourses.length > 0) {
      console.log("=================================================");
      console.log(`[DEBUG_PROFESOR] Tienes un total de ${allCourses.length} cursos cargados.`);
      console.log("[DEBUG_PROFESOR] Lista de cursos:");
      allCourses.forEach((item, index) => {
        if (item.curso) {
          console.log(`  ${index + 1}. ID: ${item.curso.id} | Título: "${item.curso.titulo}" | Estado: ${item.curso.estado}`);
        }
      });
      console.log("=================================================");
    }
  ...
  const handleManageOfferClick = (curso) => {
    setSelectedCourseForOffer(curso);
          ...
          {filteredCourses.map((item) => (
            <Grid item key={item.curso.id} xs={12} sm={6} md={6} lg={4}>
              <CourseCardEstado
                ...
                onManageOfferClick={handleManageOfferClick}
              />
            </Grid>
          ))}
        </Grid>
